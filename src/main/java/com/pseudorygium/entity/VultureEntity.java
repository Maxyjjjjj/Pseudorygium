...
public class VultureEntity extends Animal {
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new Goal() {
			...
			public void start() {
				...
				VultureEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1);
			}
			...
			public void tick() {
				...
				if (VultureEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
					VultureEntity.this.doHurtTarget(this.getServerLevel(livingentity), livingentity);
				} else {
		...
		this.targetSelector.addGoal(6, new NearestAttackableTargetGoal(this, Husk.class, false, false));
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		VultureEntity retval = PseudorygiumModEntities.VULTURE.get().create(serverWorld, EntitySpawnReason.BREEDING);
	...
	public boolean isFood(ItemStack stack) {
		return Ingredient.of(Items.ROTTEN_FLESH).test(stack);
	}
	...
	public static void init(RegisterSpawnPlacementsEvent event) {
		event.register(PseudorygiumModEntities.VULTURE.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
